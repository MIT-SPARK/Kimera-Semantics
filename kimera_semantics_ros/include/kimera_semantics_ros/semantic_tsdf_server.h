...
public:
  SemanticTsdfServer(const ros::NodeHandle& nh, const ros::NodeHandle& nh_private)
    : SemanticTsdfServer(nh, nh_private,
  ...
  {
    semantic_layer_.reset(new Layer<SemanticVoxel>(config.tsdf_voxel_size,
    ...
    tsdf_integrator_.reset(new SemanticTsdfIntegrator(
                             integrator_config,
                             semantic_config_,
                             semantic_layer_.get(),
    ...
    semantic_mesh_config_.semantic_label_color_map =
        semantic_label_to_color_.semantic_label_to_color_;
    mesh_integrator_.reset(new SemanticMeshIntegrator(
          mesh_config,
